    ...
    try:
        ...
        with open(urlcsv_relative_path, "w", newline="", encoding="utf-8") as url_file:
            ...
            for itemurl in brbproducts_url_list:
                ...
                # masterでidチェックして、無い場合は入力する。
                # 存在する場合は,datajsonにinsertしてcontinue
                # JSONファイルのパス
                json_file = "BUCHERERDATA.json"
                # jsonファイルを検索する。
                # trueなら、、日付jsonに値段と金額をinsertしてcontinueする。
                if search_json(json_file, item_id):
